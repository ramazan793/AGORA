...
from src.gghead.config.eg3d_train_arguments import EG3DTrainArguments
from src.gghead.config.gaussian_attribute import GaussianAttribute, GaussianAttributeConfig
from src.gghead.dataset.image_folder_dataset import MaskMethod, GGHeadImageFolderDatasetConfig, GGHeadMaskImageFolderDataset, DGGHeadMaskImageFolderDataset
from src.gghead.eg3d.loss import GGHeadStyleGAN2LossConfig
from src.gghead.eg3d.train_loop import launch_training
from src.gghead.env import GGHEAD_DATA_PATH
from src.gghead.model_manager.base_model_manager import GGHGeneratorType, GGHeadGANConfig, GGHeadGANOptimizerConfig, OptimizerConfig, GGHeadTrainSetup
from src.gghead.model_manager.finder import find_model_manager
from src.gghead.model_manager.gghead_model_manager import GGHeadExperimentConfig
from src.gghead.models.gaussian_discriminator import GaussianDiscriminatorConfig, DiscriminatorBlockConfig, DiscriminatorEpilogueConfig
from src.gghead.models.dyn_gghead_model import GGHeadConfig, MappingNetworkConfig, SynthesisNetworkConfig, RenderingConfig, SuperResolutionConfig
...
def main(
        ...
        resume_just_init: int = 0, # Copy just model weights and buffers
        resume_disable_rampups: int = 1, # Consider disabling rampups (schedulers) if you initializing the weights
        resume_load_D: int = 1, # Whether to load Discriminator weights or not
        ...
        use_gsm_flame_template: bool = False,
        use_flame_template_v2: bool = False,
        use_sphere_template: bool = False,
        use_plane_template: bool = False,
        use_flame2020_template: bool = True,
        use_flame_template_with_mouth: bool = False,

        # DGGHEAD
        flame_dual_discrimination: int = 0,
        dd_shape_n_components: int = 0,
        flame_rasterization_light_type: str = 'ambient',
        gen_flame_conditioning: int = 0,
        gen_cond_zero_shape: int = 0,
        use_flame_specific_double_mapping: int = 0,
        gmc_reg_prob: float = 0.0,
        gfc_double_modulation: int = 0,
        gfc_additive_condition: int = 0,
        c2_dim: int = 406,
        disc_c2_dim: int = 0,

        use_mouth_branch: int = 0,
        mouth_plane_resolution: int = 64,
        mouth_start_resolution: int = 16,
        mouth_n_vertices: int = 64,

        use_extended_uv_generation: int = 0,
        use_spade: int = 0,
        use_concat: int = 0,
        condition_nc: int = 3,

        use_deformation_branch: int = 0,
        deformation_start_resolution: int = 80,
        deformation_plane_resolution: int = 320,
        zero_out_color_residuals: int = 0,
        zero_out_opacity_residuals: int = 0,
        use_deform_mask: int = 0,
        mask_type: str = 'dejavu',
        double_condition: int = 0,
        disable_deformation_branch: int = 0,

        use_gaussian_blendshape: int = 0,
        k_blendshapes: int = 0,

        # Sample weights
        sample_weights_path: Optional[str] = None,
        use_weighted_loss: int = 0,

        use_flame_cameras: int = 0,
        use_flame_to_bfm_registration: int = 1,
        ...
        overwrite_lambda_raw_deform_gaussian_position: Optional[float] = None,
        overwrite_lambda_raw_deform_gaussian_scale: Optional[float] = None,
        overwrite_lambda_raw_deform_gaussian_opacity: Optional[float] = None,
        overwrite_lambda_id_consistency: Optional[float] = None,

        scale_offset: float = -5,

        # Dual-discrimination schedule
    ...
    use_dual_discrimination = use_dual_discrimination or flame_dual_discrimination

    if resume_run is not None and not resume_just_init:
        ...
        if overwrite_lambda_raw_deform_gaussian_position is not None:
            optimizer_config.loss_config.lambda_raw_deform_gaussian_position = overwrite_lambda_raw_deform_gaussian_position

        if overwrite_lambda_raw_deform_gaussian_scale is not None:
            optimizer_config.loss_config.lambda_raw_deform_gaussian_scale = overwrite_lambda_raw_deform_gaussian_scale

        if overwrite_lambda_raw_deform_gaussian_opacity is not None:
            optimizer_config.loss_config.lambda_raw_deform_gaussian_opacity = overwrite_lambda_raw_deform_gaussian_opacity
        ...
        if overwrite_lambda_id_consistency is not None:
            optimizer_config.loss_config.lambda_id_consistency = overwrite_lambda_id_consistency
            optimizer_config.loss_config.id_feature_resolutions = [int(x) for x in id_feature_resolutions.split(',') if x != '']

        if start_dd_kimg is not None:
            optimizer_config.loss_config.start_dd_kimg = start_dd_kimg
        ...
        # some new fields that are not in the old experiments
        generator_fields = model_config.generator_config.__dict__.keys()
        if 'use_gaussian_blendshape' not in generator_fields:
            model_config.generator_config.use_gaussian_blendshape = 0
        if 'double_condition' not in generator_fields:
            model_config.generator_config.double_condition = 0

        if resume_disable_rampups:
            ...
            optimizer_config.loss_config.gmc_reg_fade_kimg = 0  # Disable swapping rampup
            optimizer_config.freeze_generator = freeze_generator

            c.ada_kimg = 100  # Make ADA react faster at the beginning.
            c.ema_rampup = None  # Disable EMA rampup.
        
        optimizer_config.loss_config.lambda_deform_linearity_reg = lambda_deform_linearity_reg

        model_config.generator_config.mask_type = mask_type

        model_config.generator_config.rendering_config.raster_backend = raster_backend
        model_config.generator_config.disable_deformation_branch = disable_deformation_branch
    else:
        ...
        dataset_config = GGHeadImageFolderDatasetConfig(
            ...
            use_flame_cameras=use_flame_cameras)
        proxy_dataset = DGGHeadMaskImageFolderDataset(dataset_config)
        ...
        if generator_type in {'gaussians', 'triplanes'}:
            ...
            generator_config = GGHeadConfig(
                ...
                # FLAME
                use_flame_to_bfm_registration=bool(use_flame_to_bfm_registration),
                ...
                use_flame2020_template=use_flame2020_template and not use_flame_template_with_mouth,
                use_flame_template_with_mouth=use_flame_template_with_mouth,
                use_sphere_template=use_sphere_template,
                use_plane_template=use_plane_template,

                # DGGHEAD
                use_flame_rasterization=flame_dual_discrimination,
                dd_shape_n_components=dd_shape_n_components,
                flame_rasterization_light_type=flame_rasterization_light_type,
                gen_flame_conditioning=gen_flame_conditioning,
                gen_cond_zero_shape=gen_cond_zero_shape,
                use_flame_specific_double_mapping=use_flame_specific_double_mapping,
                gfc_double_modulation=gfc_double_modulation,
                gfc_additive_condition=gfc_additive_condition,
                c2_dim=c2_dim,

                use_mouth_branch=use_mouth_branch,
                mouth_plane_resolution=mouth_plane_resolution,
                mouth_start_resolution=mouth_start_resolution,
                mouth_n_vertices=mouth_n_vertices,
                
                use_extended_uv_generation=use_extended_uv_generation,
                use_spade=use_spade,
                use_concat=use_concat,
                condition_nc=condition_nc,

                use_deformation_branch=use_deformation_branch,
                deformation_start_resolution=deformation_start_resolution,
                deformation_plane_resolution=deformation_plane_resolution,
                zero_out_color_residuals=zero_out_color_residuals,
                zero_out_opacity_residuals=zero_out_opacity_residuals,
                use_deform_mask=use_deform_mask,
                mask_type=mask_type,
                double_condition=double_condition,
                disable_deformation_branch=disable_deformation_branch,

                use_gaussian_blendshape=use_gaussian_blendshape,
                k_blendshapes=k_blendshapes,
        ...
        discriminator_config.c2_dim = disc_c2_dim
        ...
        optimizer_config = GGHeadGANOptimizerConfig(
            ...
            loss_config=GGHeadStyleGAN2LossConfig(
                ...
                gmc_reg_fade_kimg=opts.gpc_reg_fade_kimg,
                gmc_reg_prob=gmc_reg_prob if gen_flame_conditioning else None,
                ...
                lambda_raw_mouth_gaussian_position=lambda_raw_mouth_gaussian_position,
                lambda_raw_gaussian_scale=lambda_raw_gaussian_scale,
                lambda_raw_mouth_gaussian_scale=lambda_raw_mouth_gaussian_scale,
                ...
                lambda_deform_linearity_reg=lambda_deform_linearity_reg,
                lambda_id_consistency=lambda_id_consistency,
                lambda_raw_deform_gaussian_position=lambda_raw_deform_gaussian_position,
                lambda_raw_deform_gaussian_scale=lambda_raw_deform_gaussian_scale,
                lambda_raw_deform_gaussian_opacity=lambda_raw_deform_gaussian_opacity,
                ...
                use_uv_reg_weights=use_uv_reg_weights,
                lambda_raw_xyz_entropy_masked=lambda_raw_xyz_entropy_masked,

                blur_masks=blur_masks,
                aug=aug,
                use_shift_augmentation=use_shift_augmentation,
                shift_aug_value=shift_aug_value,

                use_weighted_loss=use_weighted_loss,
                start_dd_kimg=start_dd_kimg,
            ),
            batch_size=batch
        )

        # Parse feature resolutions list if provided
        if id_feature_resolutions != '':
            optimizer_config.loss_config.id_feature_resolutions = [int(x) for x in id_feature_resolutions.split(',') if x != '']

        if resume_just_init and resume_disable_rampups:
            optimizer_config.loss_config.blur_init_sigma = 0  # Disable blur rampup.
            optimizer_config.loss_config.gpc_reg_fade_kimg = 0  # Disable swapping rampup
            optimizer_config.freeze_generator = freeze_generator
    ...
    train_setup = GGHeadTrainSetup(
        ...
        resume_just_init=resume_just_init,
        resume_load_D=resume_load_D,
