...
from src.gghead.eg3d.training.networks_stylegan2 import DoubleMappingNetwork
...
class GaussianDiscriminator(nn.Module):
    def __init__(self, config: GaussianDiscriminatorConfig):
        ...
        if config.c_dim > 0:
            if getattr(config, 'c2_dim', 0) > 0:
                self.use_double_mapping = True
                self.mapping = DoubleMappingNetwork(z_dim=0, c_dim=config.c_dim, c2_dim=config.c2_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, **asdict(config.mapping_network_config))
            else:
    ...
    def forward(self, img: Dict, c, update_emas=False, alpha_new_layers: float = 1, c2 = None, return_features: bool = False, feature_resolutions=None, **block_kwargs):
        if self.use_dual_discrimination:
            image_flame = img['image_flame']
            img = torch.cat([img['image'], image_flame], 1)
        else:
            img = img['image']
        ...
        if return_features:
            if feature_resolutions is not None:
                selected = {res: feat for res, feat in features.items() if res in feature_resolutions}
            else:
                selected = features
            return selected
        ...
        if self.c_dim > 0:
            ...
            if getattr(self, 'use_double_mapping', False):
                cmap = self.mapping(None, c, c2)
            else:
