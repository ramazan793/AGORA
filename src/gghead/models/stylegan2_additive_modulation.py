...
from src.gghead.eg3d.training.networks_stylegan2_additive_condition import MappingNetwork, SynthesisLayer, ToRGBLayer, Conv2dLayer
...
class GGHSynthesisBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if in_channels != 0:
            self.conv0 = SynthesisLayer(in_channels, out_channels, w_dim=w_dim, resolution=resolution, up=2,
                                        resample_filter=resample_filter, conv_clamp=conv_clamp, channels_last=self.channels_last, w2_dim=w2_dim, **layer_kwargs)
            self.num_conv += 1

        self.conv1 = SynthesisLayer(out_channels, out_channels, w_dim=w_dim, resolution=resolution,
                                    conv_clamp=conv_clamp, channels_last=self.channels_last, w2_dim=w2_dim, **layer_kwargs)
        self.num_conv += 1

        if is_last or architecture == 'skip':
            self.torgb = ToRGBLayer(out_channels, img_channels, w_dim=w_dim,
                                    conv_clamp=conv_clamp, channels_last=self.channels_last, w2_dim=w2_dim)
            self.num_torgb += 1
    ...
    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None, update_emas=False, alpha_new_layers: float = 1, w2=None, **layer_kwargs):
        ...
        # Main layers.
        if self.in_channels == 0:
            x = self.conv1(x, next(w_iter), fused_modconv=fused_modconv, w2=w2, **layer_kwargs)
        elif self.architecture == 'resnet':
            y = self.skip(x, gain=np.sqrt(0.5))
            x = self.conv0(x, next(w_iter), fused_modconv=fused_modconv, w2=w2, **layer_kwargs)
            x = self.conv1(x, next(w_iter), fused_modconv=fused_modconv, w2=w2, gain=np.sqrt(0.5), **layer_kwargs)
            x = y.add_(x)
        else:
            x = self.conv0(x, next(w_iter), fused_modconv=fused_modconv, w2=w2, **layer_kwargs)
            x = self.conv1(x, next(w_iter), fused_modconv=fused_modconv, w2=w2, **layer_kwargs)
        ...
        if self.is_last or self.architecture == 'skip':
            y = self.torgb(x, next(w_iter), fused_modconv=fused_modconv, w2=w2)
...
class GGHSynthesisNetwork(nn.Module):
    def __init__(self,
        ...
        for res in self.block_resolutions:
            ...
            block = GGHSynthesisBlock(in_channels, out_channels, w_dim=w_dim, resolution=res,
                                      img_channels=img_channels, is_last=is_last, use_fp16=use_fp16, w2_dim=w2_dim, **block_kwargs)
            if is_new_layer:
    ...
    def forward(self, ws, alpha_new_layers: float = 1, w2=None, **block_kwargs):
        block_ws = []
        ...
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            ...
            if self.pretrained_plane_resolution is not None and res > self.pretrained_plane_resolution:
                x, img = block(x, img, cur_ws, alpha_new_layers=alpha_new_layers, w2=w2, **block_kwargs)
            else:
                x, img = block(x, img, cur_ws, w2=w2, **block_kwargs)
        return img
...
class MouthGGHGenerator(nn.Module):
    def __init__(self,
        ...
        # make mouth generator smaller than the base generator
        synthesis_kwargs['channel_base'] = 32768//8//4
        synthesis_kwargs['channel_max'] = 512//2
