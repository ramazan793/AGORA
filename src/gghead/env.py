...
with env.prefixed("GGHEAD_"):
    ...
    GGHEAD_DEPENDENCIES_PATH = env("DEPENDENCIES_PATH", str((Path(REPO_ROOT_DIR).resolve() / "dependencies")))

import sys as _sys
if GGHEAD_DEPENDENCIES_PATH not in _sys.path:
    _sys.path.insert(0, GGHEAD_DEPENDENCIES_PATH)
