...
def iterate_random_labels(opts, batch_size):
    if opts.G.c_dim == 0:
        ...
        while True:
            ...
            c = torch.from_numpy(np.stack(c)).pin_memory().to(opts.device, non_blocking=True)
            yield c

def iterate_random_labels_meshes(opts, batch_size):
    dataset = opts.dataset
    ...
    while True:
        ...
        if opts.G.c_dim == 0:
            ...
            if not dataset._config.use_flame_cameras:
                c = [dataset.get_label(indices[_i]) for _i in range(batch_size)]
            else:
                c = [dataset.get_camera_parameters(indices[_i]) for _i in range(batch_size)]
            c = torch.from_numpy(np.stack(c)).pin_memory().to(opts.device, non_blocking=True)
        
        m = [dataset.get_flame_parameters(indices[_i]) for _i in range(batch_size)]
        m = torch.from_numpy(np.stack(m)).float().pin_memory().to(opts.device, non_blocking=True)
...
def compute_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, data_loader_kwargs=None, max_items=None, **stats_kwargs):
    ...
    for images, _labels, _flame_params, *_ in torch.utils.data.DataLoader(dataset=dataset, sampler=item_subset, batch_size=batch_size, **data_loader_kwargs):
        if images.shape[1] == 1:
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, batch_gen=None, **stats_kwargs):
    ...
    c_m_iter = iterate_random_labels_meshes(opts=opts, batch_size=batch_gen)
    ...
    while not stats.is_full():
        ...
        for _i in range(batch_size // batch_gen):
            ...
            c, m = next(c_m_iter)
            img = G(z=z, c=c, flame_params=m, c2=m, **opts.G_kwargs)['image']
...
#         m = [dataset.get_mesh(indices[_i]) for _i in range(batch_size)]
#         m = torch.from_numpy(np.stack(m)).float().pin_memory().to(opts.device)
...
#             z = torch.randn([batch_gen, G.z_dim], device=opts.device)
#             c, m = next(c_m_iter)
#             img = G(z=z, c=c, vertices=m, **opts.G_kwargs)['image']
#             img = (img * 127.5 + 128).clamp(0, 255).to(torch.uint8)
#             images.append(img)
#         images = torch.cat(images)
#         if images.shape[1] == 1:
