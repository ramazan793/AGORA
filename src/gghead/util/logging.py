...
class LoggerBundle:
    ...
    def set_step(self, step: int):
        assert step >= self._current_step or self._current_step == -1
    ...
    def log_metrics(self, metrics: Dict[str, Union[float, torch.Tensor]], step: Optional[int] = None, flush: bool = False):
        ...
        for metric_name, metric_value, in metrics.items():
            if isinstance(metric_value, torch.Tensor):
                ...
                raise Exception(f"Dropping metric {metric_name} as its value {metric_value} is either empty or has an unsupported format")
