...
from src.gghead.dataset.image_folder_dataset import GGHeadImageFolderDatasetConfig
from src.gghead.eg3d.loss import GGHeadStyleGAN2LossConfig
from src.gghead.env import GGHEAD_MODELS_PATH
from src.gghead.models.gaussian_discriminator import GaussianDiscriminator, GaussianDiscriminatorConfig
from src.gghead.models.gghead_model import GGHeadConfig
from src.gghead.models.dyn_gghead_model import GGHeadConfig as DGGHeadConfig
from src.gghead.util.name_builder import NameBuilder
...
class GGHeadTrainSetup(Config):
    ...
    resume_just_init: bool = False
    resume_load_D: bool = True
    total_kimg: Optional[int] = 25000
    wandb_run_id: str = implicit()

    @classmethod
    def _define_casts(cls):
        # Older saved configs stored now-bool fields as int (e.g. resume_just_init: 0). Coerce int->bool.
        return super()._define_casts() + [bool]
...
class GGHeadGANOptimizerConfig(Config):
    ...
    @classmethod
    def _define_casts(cls):
        # Older saved configs stored now-bool fields as int (e.g. loss_config.dual_discrimination: 1).
        # Coerce int->bool (applies to nested loss_config too) so resuming older checkpoints loads.
        return super()._define_casts() + [bool]
...
class GGHeadGANConfig(Config):
    ...
    generator_config: DGGHeadConfig
    ...
    @classmethod
    def _define_casts(cls):
        # Some fields that are now typed `bool` were `int` in older code, so earlier saved
        # configs store e.g. `gen_flame_conditioning: 0`. dacite (strict types) would reject
        # int for a bool field; this cast coerces int->bool so resuming older checkpoints loads.
        return super()._define_casts() + [bool]
...
class BaseModelManager(
    ...
    def __init__(self, model_type: str, run_name: str, checkpoints_sub_folder: Optional[str] = "checkpoints",
        ...
        print('trying to find the model here: ', f"{GGHEAD_MODELS_PATH}/{model_type}")
...
class BaseModelFolder(ModelFolder[_BaseModelManagerType]):
    def __init__(self, model_type: str, prefix: str):
        print('Trying to find the mode here: ', f"{GGHEAD_MODELS_PATH}/{model_type}", 'prefix: ', prefix)
